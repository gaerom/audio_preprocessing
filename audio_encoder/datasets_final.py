...
class UnAVCurationDataset(Dataset):
    def __init__(self):
        self.audio_lists = glob("../dataset/mel_train/*.npy") # UnAV-100 mel-spectrogram segment 경로로 수정
    ...
    def __getitem__(self, idx):
        ...
        for idx_audio in range(self.num_frames):
            ...
            # print(f'UnAV-100 audio segment shape: {audio_seg.shape}') # (1, 128, 153)
...
class UnAVCurationTestDataset(Dataset):
    def __init__(self):
        self.audio_lists = glob("../dataset/mel_test/*.npy")
        self.time_length = 864
        self.n_mels = 128
        self.num_frames = 5
        self.text_aug = EDA()
    ...
    def __getitem__(self, idx):
        ...
        for idx_audio in range(self.num_frames):
            audio_seg = audio_inputs[:,:,idx_audio*inter_frame:idx_audio*inter_frame+inter_frame]
            print(f'UnAV-100 audio segment shape: {audio_seg.shape}')
            _,_,seg_w = audio_seg.shape
            if seg_w >= self.frame_per_audio:
                j = random.randint(0, seg_w-self.frame_per_audio)
                audio_seg = audio_seg[:,:,j:j+self.frame_per_audio]
